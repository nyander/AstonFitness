...
(function (){
  ...
  var class_names = class_ranges.map(function(d){
    return d.name;
  });

  var is_grade_in_range = function(low,high){
    return function(d){
      return d.grade>=low && d.grade<high;
    };
  };

  var count_and_sum_effort = function(acc,d){
    //acc = {count: ..., sum: ...}
    acc.count++;
    acc.sum+=d.effort;
    return acc;
  ...
  var average_effort = class_ranges.map(function(range){
    return internet_computing_results.filter(is_grade_in_range(range.low,range.high))
             .reduce(count_and_sum_effort,{count:0,sum:0});
  }).map(function(d){return d.sum/d.count;});
  ...
  Plotly.plot('internet-computing-average-results',data,layout);
